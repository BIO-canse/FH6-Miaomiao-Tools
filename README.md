...
刷一次蓝图后，游戏结算会显示实际局内用时。这个实际局内用时必须大于 `24` 秒且小于 `28` 秒；太快会导致倍率不到 `9`，太慢会错过脚本时序。脚本会从按下 `Enter` 开始计时，等待 `37` 秒后按 `X`，这 37 秒保持不变。
...
6. 按当前车库情况自动补买目标车。
7. 自动运行刷技术点蓝图，把技术点补到 `999`。
8. 返回车库，继续下一轮。

正常情况下，开始后挂机即可。屏幕左上角浮层会显示当前大阶段、当前操作、下一步/动作串、主循环轮次、技术点、已获得超级抽奖数量、刷点进度、总耗时和失败数。
